...
for(const file of ['bridge.js','mobile.js','mobile.css','motion.js'])write(file,read('mobile/'+file));
...
for(const file of ['icon-180.png','icon-192.png','icon-512.png','photos/meadow.webp','photos/sunroom.webp','Amicro-LICENSE.txt'])write('assets/'+file,fs.readFileSync(path.join(root,'mobile/assets',file)));
