...
Implementation: shared `src/motion.js`, using the Web Animations API. No React, Tailwind, Motion runtime, CLI execution, external script, or network service is needed. This is an adaptation, not installation of the React components unchanged.

All added motion is finite and honors Reduce Motion. Both builds consume the same motion, extras, prompts, and completion styles. Mac retains native window resizing, Settings menus, and keyboard controls; the Settings zoom applies only to the phone's HTML dialog. No storage or background preference migration is needed for these visual changes.

## Completion pulse

Apple Pulse (`registry/ui/loading/apple-pulse-dots.tsx`) supplies the three-dot scale/opacity motif. Both platforms use one 900 ms pulse per dot, staggered by 150 ms, after the third priority saves successfully. It settles at full size, replaces the old completion flourish, and never loops or replays on wording edits or reload. Reduce Motion keeps static dots. The original app icon stays unchanged.
