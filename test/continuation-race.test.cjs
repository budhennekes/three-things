...
for(const surface of ['src','mobile'])test(`${surface}: a suspended extra-task opener cannot undo a concurrent Clear today`,async()=>{
 ...
 const pending=ctx.openExtra();await Promise.resolve();await ctx.changePeriod('clear');assert.equal(store.canUndoToday(key),true);
